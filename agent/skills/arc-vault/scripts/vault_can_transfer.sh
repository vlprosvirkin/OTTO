#!/usr/bin/env bash
# Usage: vault_can_transfer.sh <to_address> <amount_usdc> [chain] [vault_address] [eth_address]
...
TO="${1:?Usage: vault_can_transfer.sh <to_address> <amount_usdc> [chain] [vault_address] [eth_address]}"
